...
> [!TIP]
> It's worth enabling both security updates _and_ version updates. Security updates catch known vulnerabilities; version updates help prevent dependencies from drifting so far that upgrades become painful.
...
`daily` is the recommended default for MoJ repositories. Longer intervals reduce the frequency at which version update PRs are generated, which compounds dependency drift and delays awareness of newly available patches.

> [!NOTE]
> Schedule only affects version updates. Security alerts always bypass the schedule and are raised immediately regardless of this setting. That said, a less frequent schedule does increase the lag between a vulnerability being patched upstream and Dependabot raising a version update PR — particularly relevant for dependencies not yet covered by the GitHub Advisory Database.
...
For application dependencies, a non-`daily` schedule is really only appropriate when the repository is not yet a live product and exists solely in a development environment with no external exposure. Even then, `weekly` is a reasonable upper limit — `monthly` is best avoided. Once a service moves toward production, switching back to `daily` is a good idea.

> [!TIP]
> If PR volume feels unmanageable, [cooldown settings](#cooldown-strategy) and [grouping](#grouping-related-updates) are the right tools — they reduce noise without reducing visibility. Changing the schedule interval means potentially missing updates for longer.
...
> [!WARNING]
> When the limit is reached, Dependabot stops opening new version update PRs entirely until existing ones are merged or closed. Security alerts are subject to a separate internal limit of 10 and are unaffected, but any version update PRs that would have surfaced a vulnerable dependency before an advisory is published will be silently queued.
...
As a rule of thumb: if your total open PR count across all ecosystems regularly exceeds what your team can triage in a week, [grouping](#grouping-related-updates) and [cooldown settings](#cooldown-strategy) are more effective tools than reducing the limit — lowering the limit silently queues updates rather than making them visible.

> [!TIP]
> Keeping ecosystem entries at **5 or above** is advisable. If the current volume feels unmanageable, improving triage cadence, [grouping](#grouping-related-updates), or adjusting [cooldown settings](#cooldown-strategy) tend to be more sustainable solutions than lowering the limit.
...
> [!WARNING]
> `directory` is exclusive, not a starting point. Setting `directory: "/xyz"` means Dependabot **only** scans that path — any dependency manifests outside it will receive neither version updates nor security update PRs.
...
The practical effect is that Dependabot appears enabled, but large portions of the repository may be unmonitored — it's worth double-checking that all dependency locations are covered.
...
Limiting `directory` scope can make sense for gradual rollout on large legacy repositories, experimental sub-projects, or vendored third-party code. In those cases, documenting the omission with a review date — treating it similarly to an `ignore` rule — helps keep the gap visible and intentional.

> [!IMPORTANT]
> If the number of configured directories is fewer than the number of directories containing dependency manifests, it's worth treating that as a gap to address and documenting the reason.
...
> [!CAUTION]
> Suppressing Dependabot PRs reduces security visibility. `ignore`, `allow`, and similar rules work best when they're rare, deliberate, and temporary. [Grouping](#grouping-related-updates) and cooldown settings are usually a better first step for managing PR volume.
...
> [!CAUTION]
> - Any update raised as a **security alert** — these bypass cooldown by design, and silencing them removes an important signal
> - Updates assumed to be low-risk — exploitability assessments can change over time
> - Major version updates — the cooldown and triage process is a more reliable way to manage the effort involved

#### How to Use `ignore` Safely

When `ignore` is genuinely needed, including a comment with the justification and a target removal date makes future audits much easier. Rules without context tend to outlive their usefulness.
...
> [!WARNING]
> Using `allow` inverts Dependabot's default behaviour into an opt-in model — any dependency not matched by an `allow` rule will receive **no** security update PRs. It's worth having a documented reason before reaching for it.
...
It's a good idea to review `allow` rules at the same time as `ignore` rules — both represent deliberate gaps in update coverage.
...
The only scenario where `lockfile-only` may be acceptable is as a **short-term, explicitly tracked tactical decision** during a complex dependency resolution problem, with a tracked issue to remove it. Treating it as a permanent configuration is best avoided.
...
Retaining the `dependencies` and ecosystem labels is important. Creating `major`, `minor`, and `patch` labels is also recommended — they enable filtering and automation that makes triaging much easier. Additional labels can be configured per ecosystem:
...
Dependabot PRs have no assignees by default. Individual assignees can be added in `dependabot.yml`, but teams cannot be assigned this way. At scale, relying on individual assignees tends to create gaps — it increases PR abandonment risk and doesn't adapt well to team changes. Assignees work well for documented security on-call rotations; for everything else, CODEOWNERS is a more robust option.
...
Without CODEOWNERS, Dependabot PRs have no required reviewers and can be merged without appropriate oversight — which has both compliance and security implications worth bearing in mind.
