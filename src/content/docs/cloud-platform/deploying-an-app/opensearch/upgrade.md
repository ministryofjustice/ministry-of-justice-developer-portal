...
ingested_at: "2026-04-09T10:46:00.206Z"
...
2. Raise and merge a PR that updates the `instance_type`, `dedicated_master_type` or `warm_type` attributes for your OpenSearch domain, which is typically in `resources/opensearch.tf` in your namespace:
    ...
    The [Creating an OpenSearch domain](/docs/cloud-platform/deploying-an-app/opensearch/create) guide can help you decide which `cluster_config` to use. You should note that if your instance type is of the `t3` class, you cannot use dedicated primary nodes or warm nodes.
