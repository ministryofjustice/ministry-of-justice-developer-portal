...
async function runSession(browser, baseUrl, sessionIndex, config) {
  const stats = { sessionIndex, pageviews: 0, survey: false, errors: [] }
  ...
  try {
    ...
    // Accept analytics cookies so PostHog actually captures anything.
    const acceptButton = page.getByRole('button', { name: /accept analytics cookies/i })
    await acceptButton.click({ timeout: 5000 }).catch(() => {
      // Banner may not appear if consent was already set for some reason; carry on.
    })

    await page.waitForFunction(() => window.__posthog_initialized === true, {
      timeout: 10_000,
    }).catch(() => {
      stats.errors.push('PostHog never initialized on first page — check NEXT_PUBLIC_POSTHOG_KEY is set on this environment')
    })
